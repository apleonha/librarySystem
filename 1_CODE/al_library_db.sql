...
(2, 2, '2024-11-04 21:34:23', '2024-11-04 21:35:12', '2024-11-18'),
...
INSERT INTO `staff` (`employeeID`, `employeeFName`, `employeeLName`, `positionID`) VALUES
(1, 'Bailey', 'Tills', 1),
(2, 'Jack', 'Jamison', 2),
(3, 'Emily', 'Budd', 3),
(4, 'Veronica', 'Sams', 3),
...
INSERT INTO `user` (`cardNo`, `firstName`, `lastName`, `phone`, `email`, `balance`) VALUES
(1, 'John', 'Smith', '5555550000', 'abc@def.com', '0.00'),
(2, 'Jane', 'Doe', '5555551112', 'abb@def.com', '0.00'),
(3, 'Thomas', 'Baker', NULL, 'sadf@adflkj.com', '4.23'),
(4, 'Elizabeth', 'Johnson', '5555551122', NULL, '2.68'),
(5, 'Michael', 'Kennedy', '5555551222', 'adfs@fga.com', '0.00'),
(6, 'Brittany', 'Brown', '5555552222', 'ajfdkls@aldkjfs.com', '0.00');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `inventorytitles`  AS SELECT `inventory`.`inventoryID` AS `inventoryID`, `book`.`title` AS `title` FROM ((`inventory` join `bookinventory` on((`inventory`.`inventoryID` = `bookinventory`.`inventoryID`))) join `book` on((`bookinventory`.`ISBN` = `book`.`ISBN`))) union select `inventory`.`inventoryID` AS `inventoryID`,`cd`.`cdTitle` AS `cdTitle` from ((`inventory` join `cdinventory` on((`inventory`.`inventoryID` = `cdinventory`.`cdID`))) join `cd` on((`cdinventory`.`cdID` = `cd`.`cdID`))) union select `inventory`.`inventoryID` AS `inventoryID`,`dvd`.`dvdTitle` AS `dvdTitle` from ((`inventory` join `dvdinventory` on((`inventory`.`inventoryID` = `dvdinventory`.`dvdID`))) join `dvd` on((`dvdinventory`.`dvdID` = `dvd`.`dvdID`)))  ;
...
--
ALTER TABLE `user`
  MODIFY `cardNo` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
