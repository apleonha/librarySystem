...
<?php
	...
	$sql1 = "SELECT inventorytitles.title, holds.position FROM holds JOIN inventorytitles ON holds.inventoryID = inventorytitles.inventoryID WHERE cardNo = " . $_SESSION['cardNo'];
	$sql2 = "SELECT inventorytitles.title, checkOutHistory.checkOutDate, checkOutHistory.dateDue FROM checkOutHistory JOIN inventorytitles ON checkOutHistory.inventoryID = inventorytitles.inventoryID WHERE cardNo = " . $_SESSION['cardNo'] . "&& checkOutHistory.returnDate IS NULL";
	...
	echo "<table border = 1><tr><td>Title</td><td>Position</td></tr>";
